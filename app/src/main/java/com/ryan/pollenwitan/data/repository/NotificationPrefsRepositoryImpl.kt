...
class NotificationPrefsRepositoryImpl(
    ...
    override fun getPrefs(): Flow<NotificationPrefs> = dataStore.data.map { prefs ->
        NotificationPrefs(
    ...
    override suspend fun setMorningBriefingHour(hour: Int) {
    ...
    override suspend fun setCompoundRiskAlertsEnabled(enabled: Boolean) {
        dataStore.edit { it[Keys.COMPOUND_ENABLED] = enabled }
    }

    override suspend fun getLastBriefingDate(): LocalDate? {
        val dateStr = dataStore.data.first()[Keys.LAST_BRIEFING_DATE] ?: return null
        return LocalDate.parse(dateStr)
    }

    override suspend fun setLastBriefingDate(date: LocalDate) {
